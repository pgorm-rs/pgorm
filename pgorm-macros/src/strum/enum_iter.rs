...
pub fn enum_iter_inner(ast: &DeriveInput) -> syn::Result<TokenStream> {
    ...
    let (impl_generics, ty_generics, where_clause) = gen_.split_for_impl();
    let vis = &ast.vis;
    let type_properties = ast.get_type_properties()?;
    let strum_module_path = type_properties.crate_module_path();
    let doc_comment = format!("An iterator over the variants of [{}]", name);

    if gen_.lifetimes().count() > 0 {
    ...
    let phantom_data = if gen_.type_params().count() > 0 {
        let g = gen_.type_params().map(|param| &param.ident);
        quote! { < ( #(#g),* ) > }
    } else {
        quote! { < () > }
    };
