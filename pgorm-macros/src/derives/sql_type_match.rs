...
pub fn col_type_match(
    ...
    match col_type {
        Some(t) => quote! { pgorm::prelude::ColumnType::#t },
        None => {
            let col_type = match field_type {
                ...
                "Vec<u8>" => {
                    quote! { VarBinary(pgorm::pgorm_query::StringLen::None) }
                }
                _ => {
            ...
            if col_type.is_empty() {
                ...
                let def = quote_spanned! { field_span =>
                    std::convert::Into::<pgorm::pgorm_query::ColumnType>::into(
                        <#ty as pgorm::pgorm_query::ValueType>::column_type()
                    )
                };
                quote! { #def }
            } else {
                quote! { pgorm::prelude::ColumnType::#col_type }
            }
        }
    }
}
...
pub fn arr_type_match(
    ...
    match arr_type {
        Some(t) => quote! { pgorm::pgorm_query::ArrayType::#t },
        None => {
            ...
            if arr_type.is_empty() {
                ...
                let def = quote_spanned! { field_span =>
                    std::convert::Into::<pgorm::pgorm_query::ArrayType>::into(
                        <#ty as pgorm::pgorm_query::ValueType>::array_type()
                    )
                };
                quote! { #def }
            } else {
                quote! { pgorm::pgorm_query::ArrayType::#arr_type }
            }
        }
    }
}
