...
// [spec:pgorm:def:error.model+8/test]    the variants pgorm constructs itself, and how each renders
#[test]
fn error_variants_render_expected_messages() {
    let cases: Vec<(Error, &str)> = vec![
        ...
        (Error::NothingToSet, "The update has no columns to set"),
...
// [spec:pgorm:def:error.model+8/test]    PartialEq/Eq compare rendered messages, not payloads
#[test]
...
// [spec:pgorm:def:error.model+8/test]    ColumnFromStrError covers FromStr failures on entity columns
#[test]
...
// [spec:pgorm:def:error.model+8/test]    Result defaults to Error but still takes a foreign one
#[test]
...
// [spec:pgorm:def:error.model+8/test]    ConnectionTrait failures arrive as Postgres, rendering the server detail
#[pgorm_macros::test]
...
// [spec:pgorm:def:error.model+8/test]    DatabasePool::get surfaces pool exhaustion as Error::Pool
#[pgorm_macros::test]
...
// [spec:pgorm:def:error.model+8/test]    the builder layer's refusals convert
