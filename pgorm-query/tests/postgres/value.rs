...
// [spec:pgorm:sem:sql.value.accessor-panics/test]    `unwrap` panics on a variant mismatch
#[test]
#[should_panic]
fn unwrap_panics_on_variant_mismatch() {
    let _: i32 = Value::BigInt(Some(1)).unwrap();
}

// [spec:pgorm:sem:sql.value.accessor-panics/test]    `unwrap` panics on a nullability mismatch
#[test]
#[should_panic]
fn unwrap_panics_on_a_null_payload() {
    let _: i32 = Value::Int(None).unwrap();
}

// [spec:pgorm:sem:sql.value.accessor-panics/test]    `as_ref_*` yields None for SQL NULL of the
// right variant, but panics on a different variant
#[test]
...
// [spec:pgorm:sem:sql.value.accessor-panics/test]
#[test]
#[should_panic(expected = "not Value::Json")]
fn as_ref_json_panics_on_another_variant() {
    Value::Int(Some(1)).as_ref_json();
}

// [spec:pgorm:sem:sql.value.accessor-panics/test]
#[test]
#[should_panic(expected = "not Value::Uuid")]
fn as_ref_uuid_panics_on_another_variant() {
    Value::Int(Some(1)).as_ref_uuid();
}

// [spec:pgorm:sem:sql.value.accessor-panics/test]    chrono accessor stringifies the UTC-naive form
#[test]
...
// [spec:pgorm:sem:sql.value.accessor-panics/test]
#[test]
#[should_panic(expected = "not chrono Value")]
fn as_naive_utc_in_string_rejects_non_chrono() {
    Value::Int(Some(1)).chrono_as_naive_utc_in_string();
}

// [spec:pgorm:sem:sql.value.accessor-panics/test]    `as_ipaddr` returns the network address
#[test]
...
// [spec:pgorm:sem:sql.value.accessor-panics/test]
#[test]
#[should_panic(expected = "not Value::IpNetwork")]
fn as_ipaddr_panics_on_a_non_ipnetwork_value() {
    Value::Int(Some(1)).as_ipaddr();
}

// [spec:pgorm:sem:sql.value.accessor-panics/test]    `decimal_to_f64` goes through the payload
#[test]
