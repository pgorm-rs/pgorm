...
/// Which identity columns an INSERT is allowed to supply values for.
///
/// A `GENERATED ALWAYS AS IDENTITY` column rejects a supplied value outright,
/// and a `GENERATED BY DEFAULT` one takes it. This clause swaps those two
/// answers for the statement it sits in, which is the only way to write a
/// literal key into an `ALWAYS` column — restoring a backup, replaying a
/// migration, seeding fixtures with known ids.
// [spec:pgorm:def:sql.ast.insert+3]
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum Overriding {
    /// `OVERRIDING SYSTEM VALUE` — accept the supplied value for an `ALWAYS`
    /// identity column instead of raising `428C9`.
    SystemValue,
    /// `OVERRIDING USER VALUE` — ignore the supplied value for a `BY DEFAULT`
    /// identity column and take the sequence's next value instead.
    UserValue,
}
...
impl InsertStatement {
    ...
    /// Swap what this statement's identity columns do with a supplied value.
    ///
    /// Without it, a value written into a `GENERATED ALWAYS AS IDENTITY`
    /// column is an error rather than an override
    /// (`[spec:pgorm:req:sql.ddl.column-def+6]`), which is the point of
    ...
    /// discarding a supplied value for a `BY DEFAULT` column. The last call
    /// wins.
    ///
    /// # Examples
    ///
    ...
    ///     .values_panic([7.into(), 5.15.into()])
    ///     .to_owned();
    ///
    /// assert_eq!(
    ///     query.to_string(),
    ///     r#"INSERT INTO "glyph" ("id", "aspect") OVERRIDING SYSTEM VALUE VALUES (7, 5.15)"#
    /// );
    /// ```
    pub fn overriding(&mut self, overriding: Overriding) -> &mut Self {
