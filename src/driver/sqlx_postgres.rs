...
// use tokio_postgres::{types::ToSql, NoTls, ToStatement};
// use std::{
//     future::Future, marker::PhantomData, ops::{Deref, DerefMut}, pin::Pin, sync::Arc
// };
...
// #[repr(transparent)]
// pub struct PooledPgConnection(tokio_postgres::Client);
...
//                     tracing::error!(error=%err, "connection died");
//                 }
//             });
...
//         async {
//             if obj.0.is_closed() {
//                 Err(RecycleError::Message("connection is closed".into()))
//             } else {
//                 Ok(())
//             }
//         }
...
//         write!(f, "SqlxPostgresPoolConnection")
//     }
// }
...
//         let manager = PgPool { url: options.url };
...
//     // #[instrument(level = "trace")]
//     pub async fn execute<T>(&self, statement: &T, params: &[&(dyn ToSql + Sync)]) -> Result<u64, DbErr>
//     where
//         T: ?Sized + ToStatement,
//     {
...
//             match conn.execute(&mut **conn).await {
//                 Ok(res) => Ok(res.into()),
...
//         conn.execute(&format!("SET TRANSACTION ISOLATION LEVEL {isolation_level}"), &[]).await?;
//     }

//     if let Some(access_mode) = access_mode {
//         conn.execute(&format!("SET TRANSACTION {access_mode}"), &[]).await?;
//     }
