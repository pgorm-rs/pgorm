use crate::{ColumnDef, Error, Iterable, QueryResult, TryFromU64, TryGetError, TryGetable};
use pgorm_query::{Expr, Name, Nullable, SimpleExpr, Value, ValueType};
...
///
///     // Will be atomically generated by `DeriveActiveEnum`
///     fn try_from_value(v: &Self::Value) -> Result<Self, Error> {
///         match v.as_ref() {
///             "B" => Ok(Self::Big),
///             "S" => Ok(Self::Small),
    ...
    ($type:ident) => {
        impl ActiveEnumValue for $type {
            fn try_get_vec_by<I: RowIndex + std::fmt::Display>(
                ...
                Err(TryGetError::Db(Error::Type(format!(
