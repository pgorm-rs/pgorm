...
// [spec:pgorm:sem:codegen.entity.context+3]    extra-derive normalization
...
// [spec:pgorm:sem:codegen.entity.context+3]    extra-attribute normalization
...
impl EntityWriter {
    ...
    pub fn generate(self, context: &EntityWriterContext) -> WriterOutput {
        ...
        if !self.enums.is_empty() {
            files.push(self.write_pgorm_active_enums(
                &context.with_serde,
    ...
    pub fn write_pgorm_active_enums(
        ...
        Self::write(&mut lines, code_blocks);
        OutputFile {
            name: "pgorm_active_enums.rs".to_owned(),
            content: lines.join("\n"),
        }
    }
    ...
    pub fn write_doc_comment(lines: &mut Vec<String>) {
        ...
        let comments = vec![format!(
            "//! `pgorm` Entity, @generated by pgorm-codegen {ver}"
        )];
        ...
        entity
            ...
            .fold(
                ...
                |(mut ts, mut enums), col| {
                    if let pgorm_query::ColumnType::Enum { name, .. } = col.get_inner_col_type()
                        && !enums.contains(&name)
                    {
                        enums.push(name);
                        let enum_name = format_ident!("{}", name.to_string().to_upper_camel_case());
                        ts.extend([quote! {
                            use super::pgorm_active_enums::#enum_name;
                        }]);
                    }
                    (ts, enums)
                },
...
mod tests {
    ...
    use pgorm_query::{ColumnType, ForeignKeyAction, Name, StringLen};
    ...
    fn test_gen_import_active_enum() -> io::Result<()> {
                ...
                columns: vec![
                    ...
                    Column {
                        ...
                        col_type: ColumnType::Enum {
                            schema: None,
                            name: Name::runtime("tea_enum"),
                            variants: vec![
                                Name::runtime("everyday_tea"),
                                Name::runtime("breakfast_tea"),
                            ],
                        },
                        auto_increment: false,
                        not_null: true,
        ...
        assert_eq!(
            quote!(
                use super::pgorm_active_enums::TeaEnum;
            )
            .to_string(),
        ...
        assert_eq!(
            quote!(
                use super::pgorm_active_enums::TeaEnum;
                use super::pgorm_active_enums::TeaSize;
            )
            .to_string(),
