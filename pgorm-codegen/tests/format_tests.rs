...
fn expanded_entity_name_carries_schema_name_when_configured() {
    let plain = generate(vec![cake()], Opts::expanded());
    assert_contains(
        plain.file("cake.rs"),
...
fn expanded_relation_trait_def_matches_variants_or_panics() {
    let related = generate(vec![cake(), fruit()], Opts::expanded());
    assert_contains(
    ...
    let lonely = generate(vec![cake()], Opts::expanded());
    assert_contains(
        lonely.file("cake.rs"),
...
fn imports_sit_directly_below_the_generated_header() {
    let generated = generate(vec![cake()], Opts::expanded());
